...
gcloud beta sql instances create wfe-mysql6 \
...
nohup gcloud compute --project=$PROJECT_ID firewall-rules create default-allow-http \
    --direction=INGRESS \
    --priority=1000 \
    --network=default \
    --action=ALLOW \
...
nohup gcloud compute --project=$PROJECT_ID firewall-rules create default-allow-https \
    --direction=INGRESS \
    --priority=1000 \
    --network=default \
    --action=ALLOW \
...
nohup gcloud compute --project=$PROJECT_ID firewall-rules create default-allow-mysql \
    --direction=EGRESS \
    --priority=1000 \
    --network=default \
    --action=ALLOW \
...
# gsutil cp gs://$CODE_BUCKET-code-bucket/stenciljs_demo.tar.gz .
...
gcloud compute --project=$PROJECT_ID instances create nodejs-stenciljs \
    ...
    --metadata=startup-script=gsutil\ cp\ gs://$CODE_BUCKET-code-bucket/stenciljs_demo.tar.gz\ .$'\n'tar\ -C\ /opt/\ -zxf\ stenciljs_demo.tar.gz$'\n'chmod\ \+x\ /opt/stenciljs-db-integration-testing/bootstrap/nodejs_bootstrap.sh \
